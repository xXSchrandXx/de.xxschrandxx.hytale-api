...
use wcf\event\hytale\ExecuteEvent;
use wcf\event\hytale\GetHytaleEvent;
...
abstract class AbstractHytaleGETAction implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): JsonResponse
    {
        ...
        // gets Hytale
        $hytale = $this->getHytale($request, $response);
        $event = new GetHytaleEvent($request, $hytale, $response);
        $eventHandler->fire($event);
        $response = $event->getResponse();
        if ($response instanceof JsonResponse) {
            return $response;
        ...
        $event = new ExecuteEvent($request, $hytale, $parameters, $response);
        $eventHandler->fire($event);
        $response = $event->getResponse();
    ...
    public function prepare($request, &$response): void
    {
        ...
        // Flood control
        if (HYTALE_FLOODGATE_MAXREQUESTS > 0) {
            FloodControl::getInstance()->registerContent($this->floodgate);

            $secs = HYTALE_FLOODGATE_RESETTIME * 60;
            $time = \ceil(TIME_NOW / $secs) * $secs;
            $data = FloodControl::getInstance()->countContent($this->floodgate, new \DateInterval('PT' . HYTALE_FLOODGATE_RESETTIME . 'M'), $time);
            if ($data['count'] > HYTALE_FLOODGATE_MAXREQUESTS) {
    ...
    /**
     * Gets hytale from header
     * This method should not be modified
     * @param $request
     * @param &$response modifiable response
     * @return ?Hytale
     */
    public function getHytale($request, &$response): ?Hytale
    {
        // read header
        ...
        // search for Hytale-Entry with given user
        $hytaleList = new HytaleList();
        $hytaleList->getConditionBuilder()->add('user = ?', [$decodedArr[0]]);
        $hytaleList->readObjects();
        try {
            /** @var Hytale */
            $hytale = $hytaleList->getSingleObject();
        } catch (BadMethodCallException $e) {
            // handled by !isset
        }

        // validate Hytale-Entry exists
        if (!isset($hytale)) {
            if (ENABLE_DEBUG_MODE) {
        ...
        // check if Hytale-Entry is allowed to be used for given action
        if (isset($this->availableHytaleIDs)) {
            if (!in_array($hytale->getObjectID(), explode("\n", StringUtil::unifyNewlines($this->availableHytaleIDs)))) {
                if (ENABLE_DEBUG_MODE) {
                    $response = $this->send('Bad Request. Unknown \'Hytale-Id\'.', 400);
                } else {
                    $response = $this->send('Bad Request.', 400);
                }
                return null;
